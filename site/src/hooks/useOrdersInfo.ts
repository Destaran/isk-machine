...
export function useOrdersInfo(orders: { buy: Order[]; sell: Order[] }) {
  ...
  const weightedAverageSell =
    sell.reduce((acc, order) => acc + order.price * order.volume_remain, 0) / sellOrderVolume;

  const buyOrderVolume = buy.reduce((acc, order) => acc + order.volume_remain, 0);
  const weightedAverageBuy =
    buy.reduce((acc, order) => acc + order.price * order.volume_remain, 0) / buyOrderVolume;
